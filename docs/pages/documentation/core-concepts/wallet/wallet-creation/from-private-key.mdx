...
```typescript
import { Blockfrost, Provider } from "@mavis-sdk/lucid";
...
const blockfrost: Provider = new Blockfrost(blockfrostURL, blockfrostPID);
```
...
```typescript
import { makeWalletFromPrivateKey } from "@mavis-sdk/lucid";
