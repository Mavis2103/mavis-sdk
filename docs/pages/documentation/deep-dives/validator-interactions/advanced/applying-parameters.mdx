...
<Callout>
  `applyParamsToScript` function handles CBOR encoding internally. You can check
  encoding levels with: ```typescript const encodingLevel =
  CBOREncodingLevel(script); // Returns "double" | "single" ```
</Callout>
